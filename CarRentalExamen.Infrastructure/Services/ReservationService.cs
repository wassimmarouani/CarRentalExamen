...
public class ReservationService : IReservationService
{
    ...
    {
        var (success, error, reservation) = await CreateInternalAsync(dto.CarId, dto.CustomerId, dto.StartDate, dto.EndDate, dto.Options);
        if (!success || reservation is null)
        {
            return (false, error, null);
        }

        reservation = await LoadReservationDetail(reservation.Id);
        return (true, null, MapToDetailDto(reservation!));
    }

    public async Task<(bool Success, string? Error, ReservationDetailDto? Reservation)> CreateForCustomerAsync(ReservationCustomerCreateDto dto, int customerId)
    {
        var (success, error, reservation) = await CreateInternalAsync(dto.CarId, customerId, dto.StartDate, dto.EndDate, dto.Options);
        if (!success || reservation is null)
        {
            return (false, error, null);
        }
    ...
    public async Task<ReservationQuoteResponseDto> QuoteAsync(ReservationQuoteRequestDto request)
    {
        var validation = await ValidateReservationRequest(request.CarId, request.StartDate, request.EndDate);
        ...
        var totalDays = Math.Max(1, (request.EndDate.Date - request.StartDate.Date).Days);
        var basePrice = totalDays * validation.Car!.DailyPrice;
        var optionsPrice = request.Options.Sum(o => o.PricePerDay * o.Quantity * totalDays);
    ...
    private async Task<(bool Success, string? Error, Reservation? Reservation)> CreateInternalAsync(int carId, int customerId, DateTime startDate, DateTime endDate, IEnumerable<ReservationOptionDto> options)
    {
        var validation = await ValidateReservationRequest(carId, startDate, endDate);
        if (!validation.Success)
        {
        ...
        var basePrice = totalDays * validation.Car!.DailyPrice;
        var optionsPrice = options.Sum(o => o.PricePerDay * o.Quantity * totalDays);
        ...
        var reservation = new Reservation
        {
            ...
            Options = options.Select(o => new ReservationOption
            {
